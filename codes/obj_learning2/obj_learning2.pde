...
int count2=0;
int count = 0/*for averaging of hand*/, ref_depth;  //this counter is used for averaging the x and y coordinates of different joints
...
if(confidence==1 ) // only executed if skeleton is detected perfectly
{
...
if(count2<10)
{
  avglength=(avglength*count2+ legth.x)/(count2 +1);
  avgbreadth=(avgbreadth*count2+ breath.x)/(count2 +1);
count2++;
}
else 
{
if(abs(avglength-legth.x)< 0.1*avglength)
avglength=(avglength*count2+ legth.x)/(count2 +1);
else println("bhosda zala pan wachlo");
if(abs(avgbreadth-breath.x)< 0.1*avgbreadth)
 avgbreadth=(avgbreadth*count2+ breath.x)/(count2 +1);
else println("bhosda zala pan wachlo");
}*/
